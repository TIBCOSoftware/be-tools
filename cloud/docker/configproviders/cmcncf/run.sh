...
if ! [ -d $CERTS_PATH ]; then
  echo "Creating trustfolder directory: $CERTS_PATH"
  mkdir -p $CERTS_PATH 
fi
...
key_store_certs_generation()
{
  ...
  do    
    echo "Adding "${CNCF_SRVR_CERT[$i]}" Private keypair to keystore.jks"
...
trust_store_certs_generation()
{
  ...
  do    
    echo "Adding "${CNCF_CLNT_CERT[$i]}" to truststore.jks"
    ...
    # Copy ca.crt file to certs_path
    cp $SECRETS_PATH/${CNCF_SRVR_CERT[$i]}/ca.crt $CERTS_PATH/ca-cncf-client-$i.crt
  done  
}

if [[ -z "$CNCF_SERVER_CERT" ]]; then
  echo "WARN: Skip Converting CNCF_SERVER_CERT."
else
...
if [[ -z "$CNCF_CLIENT_CERT" ]]; then
  echo "WARN: Skip Converting CNCF_CLIENT_CERT."
else
