...
set VALIDATE_AS=%~5
::(6)ARG_HF (7)ARG_ADDONS (8)ARG_AS_LEG_VERSION (9)ARG_AS_LEG_HF
set "ARG_ADDONS="
set ARG_HF=na
set ARG_AS_LEG_VERSION=na
set ARG_AS_LEG_HF=na
...
set "IMAGE_NAME=%~1"
set "RMS_OR_TEA=na"
if !IMAGE_NAME! EQU teagent set "RMS_OR_TEA=RMS_OR_TEA"
if !IMAGE_NAME! EQU rms set "RMS_OR_TEA=RMS_OR_TEA"
...
if !IS_LESS! EQU 1 (
  ...
  if !LOCAL_INSTLR_RESULT! NEQ na (
    set ARG_AS_VERSION=!LOCAL_INSTLR_RESULT!
  )
  if !LOCAL_INSTLR_RESULT! EQU multiple (
    GOTO END-withError
  )
  ...
  if !LOCAL_INSTLR_RESULT! NEQ na (
    set ARG_AS_HF=!LOCAL_INSTLR_RESULT!
  )
  if !LOCAL_INSTLR_RESULT! EQU multiple (
    GOTO END-withError
  )
...
call :validate "!ARG_INSTALLER_LOCATION!" "!ARG_BE_VERSION!" "!ARG_ADDONS!" "!ARG_HF!" "!ARG_AS_LEG_VERSION!" "!ARG_AS_LEG_HF!" RESULT
...
if !IS_LESS! EQU 1 (
  ...
  if !ARG_AS_VERSION! NEQ na (
    call :validateFTLorACTIVESPACES "%ARG_BE_VERSION%" "%ARG_AS_VERSION%" "%ARG_AS_HF%" "%ARG_INSTALLER_LOCATION%" "as" RESULT
    if "!RESULT: =!" NEQ "0" (
...
:END
ENDLOCAL & SET %~1=%ARG_BE_VERSION%& SET %~6=%ARG_HF%& SET %~7=%ARG_ADDONS%& SET %~8=%ARG_AS_LEG_VERSION%& SET %~9=%ARG_AS_LEG_HF%& SET %~10=%ARG_JRE_VERSION%& SET %~11=%ARG_FTL_VERSION%& SET %~12=%ARG_FTL_HF%& SET %~13=%ARG_AS_VERSION%& SET %~14=%ARG_AS_HF%& SET %~15=%ERROR% 
EXIT /B 0
