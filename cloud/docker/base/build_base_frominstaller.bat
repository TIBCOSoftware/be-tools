...
set "TIBCO_BE_BASE_IMAGE_NAME=com.tibco.be" 
...
if "!ARG_DOCKERFILE!" EQU "na" if "!ARG_INSTALLERS_PLATFORM!" EQU "linux" set ARG_DOCKERFILE=Dockerfile-base-installer
if "!ARG_DOCKERFILE!" EQU "na" if "!ARG_INSTALLERS_PLATFORM!" EQU "win" set ARG_DOCKERFILE=Dockerfile-base-installer.win
if "!ARG_DOCKERFILE!" EQU "na" set ARG_DOCKERFILE=Dockerfile-base-installer 
...
echo INFO: BusinessEvents base image name - !TIBCO_BE_BASE_IMAGE_NAME!
echo ----------------------------------------------
...
echo INFO: Building docker image for TIBCO BusinessEvents Version:!ARG_VERSION! and Image Repository:!TIBCO_BE_BASE_IMAGE_NAME! and Docker file:!ARG_DOCKERFILE!
copy ..\base\!ARG_DOCKERFILE! !TEMP_FOLDER!
for %%f in (!ARG_DOCKERFILE!) do set ARG_DOCKERFILE=%%~nxf
docker build -f !TEMP_FOLDER!\!ARG_DOCKERFILE! --build-arg BE_PRODUCT_VERSION="!ARG_VERSION!" --build-arg BE_SHORT_VERSION="!SHORT_VERSION!" --build-arg BE_PRODUCT_IMAGE_VERSION="!TIBCO_BE_BASE_IMAGE_NAME!" --build-arg BE_PRODUCT_ADDONS="!ARG_ADDONS!" --build-arg BE_PRODUCT_HOTFIX="!ARG_HF!" --build-arg AS_PRODUCT_HOTFIX="!ARG_AS_HF!" --build-arg DOCKERFILE_NAME=!ARG_DOCKERFILE! --build-arg AS_VERSION="!ARG_AS_VERSION!" --build-arg AS_SHORT_VERSION="!AS_SHORT_VERSION!" --build-arg JRE_VERSION=!ARG_JRE_VERSION! --build-arg TEMP_FOLDER=!TEMP_FOLDER! --build-arg GVPROVIDERS="!ARG_GVPROVIDERS!" -t "!TIBCO_BE_BASE_IMAGE_NAME!:!ARG_VERSION!" !TEMP_FOLDER!
...
echo DONE: Docker build successful - '!TIBCO_BE_BASE_IMAGE_NAME!:!ARG_VERSION!'
