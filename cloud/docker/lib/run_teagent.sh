...
if [[ ! -z "$AWS_CONTAINER_CREDENTIALS_RELATIVE_URI" ]]; then
  ...
  ecs_metadata=$(curl 169.254.170.2/v2/metadata)
  cluster=$(echo "$ecs_metadata" | jq -r ".Cluster" | rev | cut -d '/' -f 1 | rev )
  availabilityZone=$(echo "$ecs_metadata" | jq -r ".AvailabilityZone" | rev | cut -c2- | rev)
  output="json"

  echo $cluster $availabilityZone $output 
  if [ "$8" != "nil" ]; then
    echo $8
	  cluster="$8"
	fi
...
if [[ "$9" == "AWS_ECS_FARGATE"  || "$9" == "AWS_ECS_EC2" ]]; then
  ...
  && aws configure set default.region $availabilityZone \
