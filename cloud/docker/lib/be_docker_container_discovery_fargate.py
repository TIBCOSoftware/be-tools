...
def getTaskDefinitions(clustername):
   ...
   services=get_services(clustername)
   taskDefinitions=[]
   if(services):
       for service in services:
           if(service!=None and 'ACTIVE' == service['status'] and int(service['runningCount'])>0 and service['launchType']=="FARGATE" ):
...
#get services
def get_services(clustername):
    serviceArns=[]
    response=ecs_client.list_services(cluster=clustername,launchType='FARGATE')
    if(response):
...
'''
def main(serverURL, userName, userPwd, sslEnabled, serverCert, clientCert,clustername,pythonpath,pollarinterval):
    ...
    while True:
        try:
            ...
            for task in tasks:
                if(task['launchType']=='FARGATE' and  'RUNNING'==task['desiredStatus']):
...
main(command.serverURL, command.userName, command.userPwd, command.sslEnabled, command.serverCert, command.clientCert,command.clustername,command.pythonpath,command.pollarinterval);
