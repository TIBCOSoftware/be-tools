...
USAGE+="\n\n [-d/--dockerfile]           :       Dockerfile to be used for generating image (default - Dockerfile-rms for linux container) [optional]\n" 
...
ARG_VERSION="5.6.0"
ARG_ADDONS="na"
ARG_APP_LOCATION="../../rms/bin"
ARG_BE_HOTFIX="na"
ARG_AS_HOTFIX="na"
ARG_IMAGE_VERSION="rms"
...
asPckgs=$(find $ARG_INSTALLER_LOCATION -name "$AS_REGEX")
asPckgsCnt=0
for name in "${asPckgs[@]}"
	do
		name=$(echo "${name}" | sed -e 's/^[ \t]*//')
...
asHfPckgs=$(find $ARG_INSTALLER_LOCATION -name "$AS_HF_REGEX")
asHfPckgsCnt=0
for name in "${asHfPckgs[@]}"
	do
		name=$(echo "${name}" | sed -e 's/^[ \t]*//')
		if [[ ! -z "$name" ]] ; then
			 let asHfPckgsCnt++
...
echo "INFO:APPLOCATION Data DIRECTORY : $ARG_APP_LOCATION"
